...
class ListingController extends Controller
{
    public function index(Request $request)
    {
        ...
        $listings_count = $query->get();
        $listings = $query->paginate(10)->withQueryString();

        $tags = Tag::orderBy('name')
            ->get();

        return view('listings.index', compact('listings', 'tags','listings_count'));
    }

    public function show(Listing $listing, Request $request)
    {
        $related_posts = Listing::where('is_active', true)->whereHas('tags', function ($q) use ($listing) {
            return $q->whereIn('name', $listing->tags->pluck('name'));
        })->where('id', '!=', $listing->id)->take(3)->get();

        return view('listings.show', compact('listing','related_posts'));
    }
